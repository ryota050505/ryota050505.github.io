...
面倒なのでめちゃめちゃ`any`使ってますが __絶対によろしくない__ です。  
型は気が向いたらきちんと調べたいなと言う気持ちです。
...
取得した`content`には`toc`という目次の配列が入っているので、これを使用してページ内リンクを作成することもできます。
...
できるだけ早く整ったブログを作成することが目的なら、最初から`yarn create nuxt-content-docs <project-name>`した方が良いな。。。と思いました。
